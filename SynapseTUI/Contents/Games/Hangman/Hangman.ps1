...
function loadCSV {
    Write-Host "Loading... (This could take a while)"
    $script:puzzles = Import-Csv $puzzlepath
}
loadCSV
...
While ($playagain -like 'y') {
    ...
    While (($MainBoard -like "*$ph*") -and ($incorrectcount -lt 6)) {
        ...
        Write-Host ""
        Write-Host 'Type "quit" to go back'
        Write-Host -ForegroundColor Blue "Category: $($puzzle.category)"
