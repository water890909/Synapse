...
function New-GameBoard($XSize, $YSize, $Script:MineAmount, $GameMode) {
    ...
    try{$CurrentScores = Import-Csv -Path "$gamesFolder\scores.csv"} catch {Write-Host "No data"}
...
function Test-IfWon{
    ...
    if($VisibleButtons.Count -eq 0){
        ...
        if($TopScore -eq $true -and $PlayerScore -gt 0){
            "$env:USERNAME, $PlayerScore, $Today, $GameMode" | Out-File "$gamesFolder\scores.csv" -Append
        }
...
$CurrentScores = Get-ChildItem -Path "$gamesFolder\scores.csv"  -ErrorAction SilentlyContinue

if($null -eq $CurrentScores){
    'Name,Score,Date,Mode' | Out-File "$gamesFolder\scores.csv"
    '0,0,0,Easy','0,0,0,Medium','0,0,0,Hard','0,0,0,Extreme' | Out-File "$gamesFolder\scores.csv" -Append
}else{
    ...
    if($UpdatedScores -ne $true){
        '0,0,0,Extreme' | Out-File "$gamesFolder\scores.csv" -Append
    }
    ##
}
...
function Get-HighScores {
    ...
    $CurrentScores = Import-Csv -Path "$gamesFolder\scores.csv"
    $CurrentScores | ForEach-Object {$_.Score = [int]$_.Score}
